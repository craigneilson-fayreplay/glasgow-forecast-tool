...
import { Upload, Calendar, Users, DollarSign, AlertCircle, LogOut, Cloud, Download, History, FileText } from 'lucide-react';
import { GoogleOAuthProvider, GoogleLogin } from '@react-oauth/google';
import * as XLSX from 'xlsx';

// ============================================================================
// JSPDF LOADER — loads once, all PDF exports share this promise
// ============================================================================
...
const loadJsPDF = () => {
  ...
  jsPDFLoadPromise = new Promise((resolve, reject) => {
    const loadScript = (src) => new Promise((res, rej) => {
      ...
      if (existing) {
        // Script tag exists — wait for it if still loading, resolve if done
...
const FINANCIALS = {
  ...
  CLEANER_COST: 40.00,
  AGENCY_HOURLY_RATE: 18.00,
  MANAGER_SHIFT_COST: 150.00
};

// ── Manager weekly wages by venue (actual payroll cost) ──────────────────────
const MANAGER_WEEKLY_WAGES = {
  glasgow:   1192,
  edinburgh:  615,
  newcastle: 1614
};
...
const MINIMUM_DAILY_BUDGETS = {
  ...
  edinburgh: { 0: 650, 1: 131, 2: 131, 3: 500, 4: 500, 5: 650, 6: 850 },
...
const MULTIPLIERS = {
  ...
  edinburgh: {
    0: { day14: 9.5, day7: 5.0, day3: 2.5, day1: 1.7 },
    1: { day14: 6.0, day7: 3.5, day3: 2.2, day1: 1.6 },
    2: { day14: 6.0, day7: 3.5, day3: 2.2, day1: 1.6 },
    3: { day14: 6.0, day7: 3.5, day3: 2.2, day1: 1.6 },
    4: { day14: 6.0, day7: 3.5, day3: 2.2, day1: 1.6 },
    5: { day14: 5.5, day7: 3.5, day3: 2.3, day1: 1.7 },
    6: { day14: 7.5, day7: 3.8, day3: 2.3, day1: 1.7 }
  },
  newcastle: {
    0: { day14: 8.5,  day7: 4.0,  day3: 2.5,  day1: 1.8 },
    1: { day14: 5.5,  day7: 3.0,  day3: 1.8,  day1: 1.4 },
    2: { day14: 5.5,  day7: 3.0,  day3: 1.8,  day1: 1.4 },
    3: { day14: 4.5,  day7: 2.8,  day3: 1.7,  day1: 1.3 },
    4: { day14: 6.5,  day7: 3.5,  day3: 2.0,  day1: 1.5 },
    5: { day14: 9.3,  day7: 4.5,  day3: 2.5,  day1: 1.8 },
    6: { day14: 6.6,  day7: 3.5,  day3: 2.2,  day1: 1.6 }
  },
  default: {
...
// ============================================================================
// NEWCASTLE TIERED FORECAST MODEL
// ============================================================================
...
const LookbackReport = () => {
  ...
  const [newcastleFoodRows, setNewcastleFoodRows] = useState({});
  ...
  // ── Load persisted adjustments from localStorage on mount ──────────────────
  const [manualAdjustments, setManualAdjustments] = useState(() => {
    try {
      const saved = localStorage.getItem('tfp_lookback_adjustments');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('tfp_lookback_adjustments', JSON.stringify(manualAdjustments));
    } catch (err) {
      console.warn('Could not save adjustments to localStorage:', err);
    }
  }, [manualAdjustments]);

  // ── Manager out-days per venue (persisted) ─────────────────────────────────
  // Total working days: Edinburgh=5, Glasgow=10, Newcastle=12
  // Cost = weeklyWage * (1 - outDays/totalDays)
  const MANAGER_TOTAL_DAYS = { glasgow: 10, edinburgh: 5, newcastle: 12 };

  const [managerOutDays, setManagerOutDays] = useState(() => {
    try {
      const saved = localStorage.getItem('tfp_manager_out_days');
      return saved ? JSON.parse(saved) : { glasgow: 0, edinburgh: 0, newcastle: 0 };
    } catch {
      return { glasgow: 0, edinburgh: 0, newcastle: 0 };
    }
  });

  useEffect(() => {
    try {
      localStorage.setItem('tfp_manager_out_days', JSON.stringify(managerOutDays));
    } catch (err) {
      console.warn('Could not save manager out days:', err);
    }
  }, [managerOutDays]);

  const handleBookingUpload = (e) => {
    ...
    reader.onload = (evt) => {
      ...
      const idxPeople = headers.indexOf('people');
  ...
  const handleSalesUpload = (e) => {
    ...
    if (file.name.toLowerCase().endsWith('.csv')) {
      ...
      reader.onload = (evt) => {
        ...
        for (let i = startIndex + 1; i < lines.length; i++) {
          ...
          let normalizedVenue = null;
          if (rowVenue.includes('kinning') || rowVenue.includes('glasgow')) normalizedVenue = 'glasgow';
          else if (rowVenue.includes('edinburgh')) normalizedVenue = 'edinburgh';
          else if (rowVenue.includes('newcastle')) normalizedVenue = 'newcastle';
    ...
    reader.onload = (evt) => {
      ...
      // Parse Sales Items for Newcastle food
      const saleIdDateMap = {};
      data.forEach(row => {
      ...
      if (itemsSheet) {
        ...
        itemsRows.forEach(row => {
          ...
          const valueIncVat = isToken ? 10 * quantity : (parseFloat(row[totalIdx]) || 0);
  ...
  const handleTimesheetUpload = (targetVenue) => (e) => {
    const file = e.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = (evt) => {
  ...
  const getMergedDataForVenue = React.useCallback((targetVenue) => {
    const bookingMap = {};
    const salesMap = {};
    const timesheetMap = {};
    const guestsMap = {};
    ...
    return dateRange.map(date => {
      ...
      const agencyHours = m.agencyHours || 0;
      const transfer = m.transfer || 0;

      const cleanCost = cleaningCount * FINANCIALS.CLEANER_COST;
      const agencyCost = agencyHours * FINANCIALS.AGENCY_HOURLY_RATE;

      // Employer Costs = 6.5% of staff wages
      ...
      // NOTE: Manager wages are NO LONGER included per-row.
      // They are applied as a weekly lump sum in the VenueTable footer.
      const totalLabor = r.staff + employerCosts + doorCost + djCost + cleanCost + agencyCost + transfer;
  ...
  }, [bookingRows, salesRows, timesheetRows, manualAdjustments, newcastleFoodRows]);

  const glasgowData   = useMemo(() => getMergedDataForVenue('glasgow'),   [getMergedDataForVenue]);
  const edinburghData = useMemo(() => getMergedDataForVenue('edinburgh'), [getMergedDataForVenue]);
  const newcastleData = useMemo(() => getMergedDataForVenue('newcastle'), [getMergedDataForVenue]);
  ...
  const VenueTable = React.memo(({ venueName, venueData }) => {
    ...
    const defaultWeeklyWage = MANAGER_WEEKLY_WAGES[venueKey] || 0;
    const totalDays = MANAGER_TOTAL_DAYS[venueKey] || 1;
    const outDays = managerOutDays[venueKey] ?? 0;
    const daysWorked = Math.max(0, totalDays - outDays);
    const managerWageCost = defaultWeeklyWage * (daysWorked / totalDays) * 1.15;
    ...
    const totals = venueData.reduce((acc, row) => ({
      ...
      guests: 0, ticketIncome: 0, barIncome: 0, totalIncome: 0,
      staff: 0, doorCost: 0, djCost: 0, cleanCost: 0,
      agencyCost: 0, transfer: 0, employerCosts: 0, totalLabor: 0
    });

    // Overall ratio WITHOUT manager wages (row-level sub-total)
    const subRatio = totals.totalIncome > 0 ? (totals.totalLabor / totals.totalIncome) * 100 : 0;
    // Overall ratio WITH manager wages applied
    const grandTotalLabor = totals.totalLabor + managerWageCost;
    const overallRatio = totals.totalIncome > 0 ? (grandTotalLabor / totals.totalIncome) * 100 : 0;
    ...
    const handleExportVenuePDF = async () => {
      ...
      doc.text(`Staff + Other Costs: £${totals.totalLabor.toFixed(0)}`, 80, 42);
      doc.text(`Manager Wages (${daysWorked}/${totalDays} days): £${managerWageCost.toFixed(0)}`, 150, 42);
      doc.text(`Total Labor Ratio: ${overallRatio.toFixed(1)}%`, 220, 42);
      ...
      tableData.push([
        `MANAGER WAGES (${daysWorked}/${totalDays} days of £${defaultWeeklyWage})`,
        '', '', '', '', '', '', '', '', '', '',
        `£${managerWageCost.toFixed(0)}`,
        ''
      ]);

      tableData.push([
        'GRAND TOTAL (inc. managers)',
        '', '', `£${totals.totalIncome.toFixed(0)}`,
        '', '', '', '', '', '', '',
        `£${grandTotalLabor.toFixed(0)}`,
        `${overallRatio.toFixed(1)}%`
      ]);

      doc.autoTable({
        startY: 50,
        head: [['Date','Ticket','Bar','Total Inc','Staff','Door','DJ','Clean','Agency','Transfer','Emp.Costs','Total Cost','Ratio %']],
        body: tableData,
        theme: 'grid',
        ...
        didParseCell: function(data) {
          const lastThree = [tableData.length - 3, tableData.length - 2, tableData.length - 1];
          if (lastThree.includes(data.row.index)) {
      ...
      doc.save(`lookback-${venueKey}-${new Date().toISOString().split('T')[0]}.pdf`);
    };
          ...
          <table className="w-full text-xs text-left">
            <thead className="bg-gray-50 text-gray-600 font-medium border-b">
              <tr>
                ...
                <th className="px-3 py-3 w-20 text-center bg-indigo-50 text-indigo-800">Guests</th>
            ...
            <tbody className="divide-y divide-gray-100">
                ...
                <tr key={`${venueName}-${row.date}`} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-2 py-1">
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        onBlur={(e) => handleAdjustmentChange(venueKey, row.date, 'doorCost', e.target.value)}
                  ...
                  <td className="px-2 py-1">
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        onBlur={(e) => handleAdjustmentChange(venueKey, row.date, 'djCost', e.target.value)}
                  ...
                  <td className="px-2 py-1">
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        onBlur={(e) => handleAdjustmentChange(venueKey, row.date, 'cleaningCount', e.target.value)}
                  ...
                  <td className="px-2 py-1">
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        onBlur={(e) => handleAdjustmentChange(venueKey, row.date, 'agencyHours', e.target.value)}
                  ...
                  <td className="px-2 py-1 border-r border-gray-100">
                    <input
                      ...
                      onBlur={(e) => handleAdjustmentChange(venueKey, row.date, 'transfer', e.target.value)}
              ...
              {/* ── SUBTOTALS ROW (excluding manager wages) ── */}
              <tr className="bg-gray-100 border-t-2 border-gray-300 font-bold">
                <td className="px-3 py-3 text-gray-700 text-xs">SUBTOTALS</td>
                <td className="px-3 py-3 text-center bg-indigo-100 text-indigo-900">{totals.guests}</td>
                ...
                <td className={`px-3 py-3 text-right font-bold ${subRatio > 30 ? 'text-red-600' : 'text-green-600'}`}>
                  {subRatio.toFixed(1)}%
                </td>
              </tr>
              ...
              <tr className="bg-purple-50 border-t border-purple-200">
                <td colSpan={4} className="px-3 py-3">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="font-bold text-purple-800 text-xs whitespace-nowrap">
                      👔 Manager Wages
                    </span>
                    <span className="text-xs text-purple-600">
                      Full week: £{defaultWeeklyWage.toLocaleString()} ({totalDays} days)
                    </span>
                    <div className="flex items-center gap-2">
                      <label className="text-xs text-purple-700 font-medium whitespace-nowrap">Manager out days:</label>
                      <input
                        type="number"
                        min="0"
                        max={totalDays}
                        step="1"
                        defaultValue={outDays}
                        onBlur={(e) => {
                          const v = Math.min(totalDays, Math.max(0, parseInt(e.target.value) || 0));
                          setManagerOutDays(prev => ({ ...prev, [venueKey]: v }));
                        }}
                        key={`${venueKey}-out-days-${outDays}`}
                        className="w-12 text-center border border-purple-300 rounded px-1 py-0.5 text-purple-800 font-bold bg-white text-xs"
                      />
                      <span className="text-purple-600 text-xs">of {totalDays}</span>
                    </div>
                    <span className="text-xs text-purple-500 italic">
                      → {daysWorked} day{daysWorked !== 1 ? 's' : ''} worked = £{managerWageCost.toFixed(0)} this week (inc. 15% emp. costs)
                    </span>
                  </div>
                </td>
                <td className="px-3 py-3" colSpan={7}></td>
                <td className="px-3 py-3 text-right font-bold text-purple-800 bg-purple-100">
                  £{managerWageCost.toFixed(0)}
                </td>
                <td className="px-3 py-3 text-right text-purple-600 text-xs italic">incl. below</td>
              </tr>

              {/* ── GRAND TOTAL ROW (inc. manager wages) ── */}
              <tr className="bg-indigo-50 border-t-2 border-indigo-300 font-bold">
                <td colSpan={4} className="px-3 py-3 text-indigo-900 font-bold">
                  TOTAL (inc. manager wages)
                </td>
                <td className="px-3 py-3 text-right font-bold text-blue-900 border-r border-indigo-300">
                  £{totals.totalIncome.toFixed(0)}
                </td>
                <td colSpan={7} className="px-3 py-3"></td>
                <td className="px-3 py-3 text-right font-bold text-red-900 bg-red-100">
                  £{grandTotalLabor.toFixed(0)}
                </td>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-orange-50 border border-orange-200 rounded-xl p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {['glasgow', 'edinburgh', 'newcastle'].map(v => (
            <div key={v} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
              <h3 className="font-bold text-gray-700 mb-3 flex items-center gap-2">
                <History size={18} className="text-orange-600"/> {v.charAt(0).toUpperCase() + v.slice(1)}
              </h3>
              <p className="text-xs text-gray-500 mb-3">Timesheet CSV</p>
              <input type="file" accept=".csv" onChange={handleTimesheetUpload(v)} className="text-xs w-full file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100" />
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 flex items-center gap-3 text-sm text-green-800">
        ...
        <span>Your manual adjustments (door, DJ, cleaning, agency, transfer, manager %) are automatically saved and will be restored next time you open this tool.</span>
        <button
          onClick={() => {
            if (window.confirm('Clear all saved adjustments? This cannot be undone.')) {
              localStorage.removeItem('tfp_lookback_adjustments');
              localStorage.removeItem('tfp_manager_out_days');
              setManualAdjustments({});
              setManagerOutDays({ glasgow: 0, edinburgh: 0, newcastle: 0 });
            }
          }}
          className="ml-auto text-xs px-3 py-1 bg-white border border-green-300 rounded-lg hover:bg-red-50 hover:border-red-300 hover:text-red-700 transition-colors whitespace-nowrap"
        >
          Clear saved data
        </button>
      </div>
      ...
      {(glasgowData.length > 0 || edinburghData.length > 0 || newcastleData.length > 0) && (
        <div className="flex justify-end mb-4">
          <button
            onClick={async () => {
              const venues = [
                { name: 'Glasgow',   data: glasgowData,   key: 'glasgow' },
                { name: 'Edinburgh', data: edinburghData, key: 'edinburgh' },
                { name: 'Newcastle', data: newcastleData, key: 'newcastle' }
              ...
              for (const venue of venues) {
                if (!isFirstVenue) doc.addPage();
                isFirstVenue = false;

                const vOutDays = managerOutDays[venue.key] ?? 0;
                const vTotalDays = MANAGER_TOTAL_DAYS[venue.key] || 1;
                const vDaysWorked = Math.max(0, vTotalDays - vOutDays);
                const defaultWage = MANAGER_WEEKLY_WAGES[venue.key] || 0;
                const managerWageCost = defaultWage * (vDaysWorked / vTotalDays) * 1.15;
                ...
                const totals = venue.data.reduce((acc, row) => ({
                  ...
                  employerCosts: acc.employerCosts + row.employerCosts,
                  totalLabor: acc.totalLabor + row.totalLabor
                }), { ticketIncome: 0, barIncome: 0, totalIncome: 0, staff: 0, doorCost: 0, djCost: 0, cleanCost: 0, agencyCost: 0, transfer: 0, employerCosts: 0, totalLabor: 0 });
                ...
                doc.setFontSize(10);
                doc.setTextColor(0, 0, 0);
                doc.text(`Income: £${totals.totalIncome.toFixed(0)}  |  Staff & Other: £${totals.totalLabor.toFixed(0)}  |  Manager Wages (${vDaysWorked}/${vTotalDays} days): £${managerWageCost.toFixed(0)}  |  Total Ratio: ${overallRatio.toFixed(1)}%`, 15, 35);
                ...
                tableData.push(['SUBTOTALS', `£${totals.ticketIncome.toFixed(0)}`, `£${totals.barIncome.toFixed(0)}`, `£${totals.totalIncome.toFixed(0)}`, `£${totals.staff.toFixed(0)}`, `£${totals.doorCost.toFixed(0)}`, `£${totals.djCost.toFixed(0)}`, `£${totals.cleanCost.toFixed(0)}`, `£${totals.agencyCost.toFixed(0)}`, `£${totals.transfer.toFixed(0)}`, `£${totals.employerCosts.toFixed(0)}`, `£${totals.totalLabor.toFixed(0)}`, `${subRatio.toFixed(1)}%`]);
                tableData.push([`MANAGER WAGES (${vDaysWorked}/${vTotalDays} days of £${defaultWage})`, '', '', '', '', '', '', '', '', '', '', `£${managerWageCost.toFixed(0)}`, '']);
                tableData.push(['GRAND TOTAL (inc. managers)', '', '', `£${totals.totalIncome.toFixed(0)}`, '', '', '', '', '', '', '', `£${grandTotalLabor.toFixed(0)}`, `${overallRatio.toFixed(1)}%`]);
                ...
                doc.autoTable({
                  ...
                  didParseCell: function(data) {
                    ...
                    if (data.row.index >= n - 3) {
                      data.cell.styles.fillColor = [243, 244, 246];
                      data.cell.styles.fontStyle = 'bold';
                    }
                    if (data.row.index === n - 1) {
                      data.cell.styles.fillColor = [224, 231, 255];
                    }
                  }
                });
              ...
              for (let i = 1; i <= pageCount; i++) {
                ...
                doc.setFontSize(8);
                doc.setTextColor(156, 163, 175);
                doc.text(`Page ${i} of ${pageCount} | All Venues Lookback | Generated ${today}`, doc.internal.pageSize.width / 2, doc.internal.pageSize.height - 10, { align: 'center' });
              }
...
const NewcastleFoodVendorReport = () => {
  ...
  const isFoodToken = (row, headers) => {
    ...
    const category = String(row[categoryIdx] || '');
    if (!category.substring(0, 4).toLowerCase().includes('food')) return false;
  ...
  const processData = async () => {
    ...
    try {
      ...
      reader.onload = async (evt) => {
        ...
        if (!salesSheet) salesSheet = wb.Sheets[wb.SheetNames[0]];
        ...
        if (saleIdIdx === -1 || outletIdx === -1) throw new Error('Sales tab missing required columns: Sale ID, Outlet');
        ...
        salesRows.forEach(row => {
          ...
          if (saleId && outlet) saleIdMap[saleId] = { outlet, date: saleDate };
        });

        let itemsSheet = wb.Sheets['Sales Items'];
        if (!itemsSheet) {
          const itemSheetName = wb.SheetNames.find(name => name.toLowerCase().includes('item'));
          if (itemSheetName) itemsSheet = wb.Sheets[itemSheetName];
        }
        if (!itemsSheet) throw new Error('Could not find Sales Items sheet.');
        ...
        if (itemSaleIdIdx === -1 || categoryIdx === -1 || quantityIdx === -1 || totalIdx === -1) throw new Error('Sales Items tab missing required columns');
        if (productNameIdx === -1) throw new Error('Sales Items tab missing "Product Name" column');
        ...
        itemsRows.forEach(row => {
          ...
          if (isToken) totalTokens += quantity * 10;
          else totalCardSales += value;

          if (!itemSales[productName]) itemSales[productName] = { quantity: 0, tokenIncome: 0, purchasedIncome: 0, total: 0 };
          itemSales[productName].quantity += quantity;
          if (isToken) itemSales[productName].tokenIncome += value;
          else itemSales[productName].purchasedIncome += value;
          itemSales[productName].total += value;

          if (dateKey) {
            if (!dailySales[dateKey]) dailySales[dateKey] = {};
            if (!dailySales[dateKey][productName]) dailySales[dateKey][productName] = { quantity: 0, tokenIncome: 0, purchasedIncome: 0, total: 0 };
            dailySales[dateKey][productName].quantity += quantity;
            if (isToken) dailySales[dateKey][productName].tokenIncome += value;
            else dailySales[dateKey][productName].purchasedIncome += value;
            dailySales[dateKey][productName].total += value;
          }
        });

        if (foodItemsFound === 0) throw new Error(`No food items found for Newcastle. Found ${newcastleItemsFound} Newcastle items total.`);

        const salesArray = Object.entries(itemSales).map(([product, data]) => ({
          item: product, quantity: data.quantity, tokenIncome: data.tokenIncome, purchasedIncome: data.purchasedIncome, total: data.total
        ...
        const dailyBreakdown = Object.entries(dailySales).map(([date, products]) => {
          ...
          return { date, items: itemsArray, dayTotal };
        ...
        setReportData({ sales: salesArray, dailyBreakdown, totalFoodTakings, vendorPayment, commission, totalTokens, totalCardSales, weekStart: earliestDate, weekEnd: latestDate });
        setLoading(false);
      };
      reader.readAsBinaryString(file);
    } catch (err) {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      doc.setFontSize(24); doc.setTextColor(243, 0, 80); doc.setFont('helvetica', 'bold');
      ...
      if (reportData.weekStart && reportData.weekEnd) {
        ...
        doc.text(`Week: ${reportData.weekStart.toLocaleDateString('en-GB')} - ${reportData.weekEnd.toLocaleDateString('en-GB')}`, pageWidth / 2, 38, { align: 'center' });
      }
      ...
      const summaryLines = [
        { label: 'Food Card Sales:', value: `£${reportData.totalCardSales.toFixed(2)}`, bold: false },
        { label: 'Food Token Sales:', value: `£${reportData.totalTokens.toFixed(2)}`, bold: false },
      ...
      summaryLines.forEach(line => {
        ...
        doc.setTextColor(...(line.color || [80,80,80]));
      ...
      if (reportData.dailyBreakdown && reportData.dailyBreakdown.length > 0) {
        reportData.dailyBreakdown.forEach((day) => {
          ...
          doc.setFontSize(11); doc.setFont('helvetica', 'bold'); doc.setTextColor(243,0,80);
          doc.text(`${dayName} - £${day.dayTotal.toFixed(2)}`, 15, currentY);
          currentY += 3;

          const dayTableData = day.items.map(item => [item.item, item.quantity.toString(), `£${item.tokenIncome.toFixed(2)}`, `£${item.purchasedIncome.toFixed(2)}`, `£${item.total.toFixed(2)}`]);
          dayTableData.push(['Totals', day.items.reduce((s,i)=>s+i.quantity,0).toString(), `£${day.items.reduce((s,i)=>s+i.tokenIncome,0).toFixed(2)}`, `£${day.items.reduce((s,i)=>s+i.purchasedIncome,0).toFixed(2)}`, `£${day.dayTotal.toFixed(2)}`]);
          ...
          doc.autoTable({
            ...
            body: dayTableData,
            theme: 'plain',
            headStyles: { fillColor: [243,0,80], textColor: [255,255,255], fontStyle: 'bold', fontSize: 8 },
            styles: { fontSize: 8, cellPadding: 1.5, fillColor: [253,254,233] },
            alternateRowStyles: { fillColor: [253,254,233] },
            columnStyles: { 0: { cellWidth: 80 }, 1: { halign: 'center', cellWidth: 20 }, 2: { halign: 'right', cellWidth: 25 }, 3: { halign: 'right', cellWidth: 25 }, 4: { halign: 'right', cellWidth: 25 } },
            margin: { left: 20 },
            didParseCell: (data) => {
              if (data.row.index === dayTableData.length - 1) { data.cell.styles.fontStyle = 'bold'; data.cell.styles.fillColor = [220,220,220]; }
            },
            didAddPage: () => { doc.setFillColor(253,254,233); doc.rect(0,0,pageWidth,pageHeight,'F'); }
          });
      ...
      if (currentY > pageHeight - 80) {
        doc.addPage();
        doc.setFillColor(253, 254, 233); doc.rect(0, 0, pageWidth, pageHeight, 'F');
        currentY = 20;
      }
      ...
      const tableData = reportData.sales.map(item => [item.item, item.quantity.toString(), `£${item.tokenIncome.toFixed(2)}`, `£${item.purchasedIncome.toFixed(2)}`, `£${item.total.toFixed(2)}`]);
      ...
      doc.autoTable({
        ...
        body: tableData,
        theme: 'grid',
        headStyles: { fillColor: [14,201,129], textColor: [255,255,255], fontStyle: 'bold' },
        styles: { fontSize: 9, fillColor: [253,254,233] },
        alternateRowStyles: { fillColor: [248,250,225] },
        columnStyles: { 0: { cellWidth: 70 }, 1: { halign: 'center', cellWidth: 25 }, 2: { halign: 'right', cellWidth: 25 }, 3: { halign: 'right', cellWidth: 25 }, 4: { halign: 'right', cellWidth: 30 } },
        didAddPage: () => { doc.setFillColor(253,254,233); doc.rect(0,0,pageWidth,pageHeight,'F'); }
      });
  ...
  return (
    <div style={{ backgroundColor: POPCORN_WHITE, minHeight: '100vh', padding: '20px' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ background: `linear-gradient(135deg, ${GLASGA_RED} 0%, ${BUCKY_GREEN} 100%)`, borderRadius: '12px', padding: '30px', marginBottom: '30px', color: 'white' }}>
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '12px', border: '2px dashed #0ec981', padding: '40px', textAlign: 'center', marginBottom: '30px' }}>
          <Upload size={48} style={{ color: BUCKY_GREEN, margin: '0 auto 20px' }} />
          <h3 style={{ marginBottom: '10px' }}>Upload POS Sales Report</h3>
          <p style={{ color: '#666', marginBottom: '20px', fontSize: '14px' }}>Upload your weekly SumUp POS export (Excel format)</p>
          <input type="file" accept=".xlsx,.xls" onChange={(e) => { setFile(e.target.files[0]); setReportData(null); setError(null); }} style={{ display: 'none' }} id="food-vendor-file-upload" />
          <label htmlFor="food-vendor-file-upload" style={{ display: 'inline-block', backgroundColor: BUCKY_GREEN, color: 'white', padding: '12px 30px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>Choose File</label>
          {file && <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#e8f5e9', borderRadius: '8px', border: '1px solid #0ec981' }}><p style={{ margin: 0, color: '#2e7d32' }}>✓ {file.name}</p></div>}
          {file && <button onClick={processData} disabled={loading} style={{ marginTop: '20px', backgroundColor: GLASGA_RED, color: 'white', padding: '12px 30px', borderRadius: '8px', border: 'none', cursor: loading ? 'not-allowed' : 'pointer', fontWeight: 'bold', opacity: loading ? 0.6 : 1 }}>{loading ? 'Processing...' : 'Generate Report'}</button>}
          {error && <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#ffebee', borderRadius: '8px', border: '1px solid #f30050', color: '#c62828' }}>{error}</div>}
        </div>
        ...
        {reportData && (
          <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '30px', marginBottom: '30px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <button onClick={generatePDF} style={{ backgroundColor: GLASGA_RED, color: 'white', padding: '12px 24px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <Download size={20} /> Download PDF
              </button>
            </div>

            {reportData.weekStart && reportData.weekEnd && (
              <div style={{ backgroundColor: '#e3f2fd', padding: '15px', borderRadius: '8px', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <Calendar size={20} style={{ color: '#1976d2' }} />
                <span style={{ fontWeight: 'bold' }}>Week: {reportData.weekStart.toLocaleDateString('en-GB')} - {reportData.weekEnd.toLocaleDateString('en-GB')}</span>
              </div>
            )}

            <div style={{ backgroundColor: '#f5f5f5', padding: '20px', borderRadius: '8px', border: '2px solid #e0e0e0', marginBottom: '30px' }}>
              <h3 style={{ marginTop: 0 }}>💰 Financial Summary</h3>
              <div style={{ display: 'grid', gap: '10px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Food Card Sales:</span><span style={{ fontWeight: 'bold' }}>£{reportData.totalCardSales.toFixed(2)}</span></div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Food Token Sales:</span><span style={{ fontWeight: 'bold' }}>£{reportData.totalTokens.toFixed(2)}</span></div>
                <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '10px', borderTop: '2px solid #ccc', fontSize: '18px' }}><span style={{ fontWeight: 'bold' }}>Total Food Takings (inc VAT):</span><span style={{ fontWeight: 'bold' }}>£{reportData.totalFoodTakings.toFixed(2)}</span></div>
                <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '15px', marginTop: '5px', fontSize: '18px', color: BUCKY_GREEN }}><span style={{ fontWeight: 'bold' }}>Vendor Payment (80%):</span><span style={{ fontWeight: 'bold' }}>£{reportData.vendorPayment.toFixed(2)}</span></div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '18px', color: GLASGA_RED }}><span style={{ fontWeight: 'bold' }}>Your Commission (20%):</span><span style={{ fontWeight: 'bold' }}>£{reportData.commission.toFixed(2)}</span></div>
              </div>
            </div>
            ...
            {reportData.dailyBreakdown && reportData.dailyBreakdown.length > 0 && (
              <div style={{ marginBottom: '30px' }}>
                ...
                {reportData.dailyBreakdown.map((day, dayIdx) => {
                  ...
                  return (
                    <div key={dayIdx} style={{ backgroundColor: dayIdx % 2 === 0 ? '#f9f9f9' : 'white', padding: '15px', borderRadius: '8px', marginBottom: '10px', border: '1px solid #e0e0e0' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', paddingBottom: '10px', borderBottom: '2px solid ' + GLASGA_RED }}>
                        <span style={{ fontWeight: 'bold', fontSize: '16px' }}>{dayName}</span>
                        <span style={{ fontWeight: 'bold', fontSize: '16px', color: BUCKY_GREEN }}>£{day.dayTotal.toFixed(2)}</span>
                      </div>
                      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead><tr style={{ backgroundColor: '#f5f5f5' }}><th style={{ padding: '8px', textAlign: 'left', fontSize: '12px' }}>Item</th><th style={{ padding: '8px', textAlign: 'center', fontSize: '12px' }}>Qty</th><th style={{ padding: '8px', textAlign: 'right', fontSize: '12px' }}>Token</th><th style={{ padding: '8px', textAlign: 'right', fontSize: '12px' }}>Purchased</th><th style={{ padding: '8px', textAlign: 'right', fontSize: '12px' }}>Total</th></tr></thead>
                        <tbody>
                          {day.items.map((item, itemIdx) => (
                            <tr key={itemIdx}>
                              <td style={{ padding: '6px', fontSize: '13px' }}>{item.item}</td>
                              <td style={{ padding: '6px', textAlign: 'center', fontSize: '13px', fontWeight: 'bold' }}>{item.quantity}</td>
                              <td style={{ padding: '6px', textAlign: 'right', fontSize: '13px' }}>£{item.tokenIncome.toFixed(2)}</td>
                              <td style={{ padding: '6px', textAlign: 'right', fontSize: '13px' }}>£{item.purchasedIncome.toFixed(2)}</td>
                              <td style={{ padding: '6px', textAlign: 'right', fontSize: '13px', fontWeight: 'bold' }}>£{item.total.toFixed(2)}</td>
                            </tr>
                          ))}
                        </tbody>
                        <tfoot><tr style={{ borderTop: '2px solid #ccc', backgroundColor: '#f0f0f0' }}><td style={{ padding: '8px', fontSize: '13px', fontWeight: 'bold' }}>Totals</td><td style={{ padding: '8px', textAlign: 'center', fontSize: '13px', fontWeight: 'bold' }}>{day.items.reduce((s,i)=>s+i.quantity,0)}</td><td style={{ padding: '8px', textAlign: 'right', fontSize: '13px', fontWeight: 'bold', color: '#c00' }}>£{day.items.reduce((s,i)=>s+i.tokenIncome,0).toFixed(2)}</td><td style={{ padding: '8px', textAlign: 'right', fontSize: '13px', fontWeight: 'bold', color: '#c00' }}>£{day.items.reduce((s,i)=>s+i.purchasedIncome,0).toFixed(2)}</td><td style={{ padding: '8px', textAlign: 'right', fontSize: '13px', fontWeight: 'bold', color: '#2a7a2a' }}>£{day.dayTotal.toFixed(2)}</td></tr></tfoot>
                      </table>
                    </div>
                  );
                })}
            ...
            <div style={{ overflowX: 'auto', marginBottom: '10px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr style={{ backgroundColor: BUCKY_GREEN, color: 'white' }}><th style={{ padding: '12px', textAlign: 'left', borderBottom: '2px solid #ddd' }}>Item</th><th style={{ padding: '12px', textAlign: 'center', borderBottom: '2px solid #ddd' }}>Total Quantity</th><th style={{ padding: '12px', textAlign: 'right', borderBottom: '2px solid #ddd' }}>Token</th><th style={{ padding: '12px', textAlign: 'right', borderBottom: '2px solid #ddd' }}>Purchased</th><th style={{ padding: '12px', textAlign: 'right', borderBottom: '2px solid #ddd' }}>Total</th></tr></thead>
                <tbody>
                  {reportData.sales.map((item, idx) => (
                    <tr key={idx} style={{ backgroundColor: idx % 2 === 0 ? '#f9f9f9' : 'white' }}>
                      <td style={{ padding: '10px', borderBottom: '1px solid #ddd' }}>{item.item}</td>
                      <td style={{ padding: '10px', textAlign: 'center', borderBottom: '1px solid #ddd', fontWeight: 'bold' }}>{item.quantity}</td>
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '25px', border: '1px solid #e0e0e0' }}>
          ...
          <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#fff3e0', borderRadius: '8px', border: '1px solid #ff6f00' }}>
            <p style={{ margin: '0 0 10px 0', fontSize: '14px' }}><strong>💡 How it works:</strong></p>
...
const Glasgow14DayForecast = () => {
  ...
  const projection = useMemo(() => {
    ...
    const thisWeek = processedDays.filter(d => d.date <= new Date(getMonday(new Date()).getTime() + 6 * 86400000 + 86399999));
    const nextWeek = processedDays.filter(d => d.date > new Date(getMonday(new Date()).getTime() + 6 * 86400000 + 86399999));
    ...
    const calcSummary = (days) => {
      ...
      return { revenue: rev, laborCost: cost, budget: bud, laborPct: rev > 0 ? (cost/rev)*100 : 0, covers: days.reduce((s, d) => s + d.forecastCovers, 0) };
    };
    ...
    try {
      ...
      const parseCSVLine = (line) => {
        ...
        for (let i = 0; i < line.length; i++) {
          ...
          if (char === ',' && !inQuote) { values.push(current.trim()); current = ''; }
          else { current += char; }
        }
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (idxVenue > -1 && row[idxVenue]) rowVenue = row[idxVenue].toLowerCase();
        ...
        if (!dateStr && idxDateAlt !== -1) dateStr = row[idxDateAlt].replace(/"/g, '').trim();
        ...
        const isTicketProduct = productStr.includes('ticket') || productStr.includes('lucky duckers') || productStr.includes('adults ticket') || productStr.includes('kids ticket');
        const rawAmount = idxAmount > -1 ? parseFloat(row[idxAmount]) || 0 : 0;
        let calculatedBookingRevenue = 0;
        if (isTicketProduct) calculatedBookingRevenue = people * FINANCIALS.GAME_TICKET_PRICE;
        else calculatedBookingRevenue = rawAmount / 1.2;

        const aggKey = `${dateKey}|${rowVenue}`;
        if (!aggregation[aggKey]) aggregation[aggKey] = { dateKey, venue: rowVenue, regular: 0, large: 0, regularRevenue: 0, largeRevenue: 0 };
      ...
      const parsed = Object.values(aggregation).map(v => {
        ...
        return { dateKey: v.dateKey, venue: v.venue, regularCovers: v.regular, largeGroupCovers: v.large, people: v.regular + v.large, bookedRevenue: v.regularRevenue + adjustedLargeRevenue };
      });

      setUploadStats({ total: lines.length - 1, valid: parsed.length });
      if (parsed.length === 0) throw new Error(`No valid bookings found.`);
      setRawData(parsed);
  ...
  const handleExport = () => {
    ...
    const rows = allDays.map(d => [d.dateKey, d.dayName, d.forecastCovers, d.revenue.toFixed(2), d.budget.toFixed(2), d.laborCost.toFixed(2), (d.budget - d.laborCost).toFixed(2)]);
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      alert('Failed to load PDF library. Check your internet connection and try again.');
      return;
    }
    ...
    doc.setFontSize(14); doc.setTextColor(0,0,0); doc.text('This Week Summary', 15, 35);
    doc.setFontSize(10); doc.setTextColor(107,114,128);
    doc.text(`Forecast Covers: ${projection.summaries.thisWeek.covers}`, 15, 42);
    doc.text(`Est. Revenue: £${projection.summaries.thisWeek.revenue.toLocaleString()}`, 80, 42);
    doc.text(`Budget: £${projection.summaries.thisWeek.budget.toLocaleString()}`, 150, 42);
    doc.text(`Variance: £${(projection.summaries.thisWeek.budget - projection.summaries.thisWeek.laborCost).toFixed(0)}`, 210, 42);

    doc.setFontSize(14); doc.setTextColor(0,0,0); doc.text('Next Week Summary', 15, 55);
    doc.setFontSize(10); doc.setTextColor(107,114,128);
    doc.text(`Forecast Covers: ${projection.summaries.nextWeek.covers}`, 15, 62);
    doc.text(`Est. Revenue: £${projection.summaries.nextWeek.revenue.toLocaleString()}`, 80, 62);
    doc.text(`Budget: £${projection.summaries.nextWeek.budget.toLocaleString()}`, 150, 62);
    doc.text(`Variance: £${(projection.summaries.nextWeek.budget - projection.summaries.nextWeek.laborCost).toFixed(0)}`, 210, 62);

    doc.setFontSize(12); doc.setTextColor(0,0,0); doc.text('This Week - Daily Breakdown', 15, 75);
    const thisWeekData = projection.thisWeek.map(d => [d.dateKey, d.dayName, d.forecastCovers, d.staffHours, `£${d.revenue.toFixed(0)}`, `£${d.budget.toFixed(0)}`, `£${d.laborCost.toFixed(0)}`, `£${(d.budget - d.laborCost).toFixed(0)}`, d.weather ? `${d.weather.temp}°C` : 'N/A']);
    doc.autoTable({ startY: 80, head: [['Date','Day','Covers','Hrs','Revenue','Budget','Labor','Variance','Weather']], body: thisWeekData, theme: 'grid', headStyles: { fillColor: [99,102,241], textColor: 255 }, styles: { fontSize: 8, cellPadding: 2 }, columnStyles: { 0: { cellWidth: 23 }, 1: { cellWidth: 18 }, 2: { cellWidth: 16, halign: 'right' }, 3: { cellWidth: 14, halign: 'right' }, 4: { cellWidth: 24, halign: 'right' }, 5: { cellWidth: 24, halign: 'right' }, 6: { cellWidth: 24, halign: 'right' }, 7: { cellWidth: 24, halign: 'right' }, 8: { cellWidth: 30 } } });

    const nextWeekStartY = doc.lastAutoTable.finalY + 10;
    doc.setFontSize(12); doc.text('Next Week - Daily Breakdown', 15, nextWeekStartY);
    const nextWeekData = projection.nextWeek.map(d => [d.dateKey, d.dayName, d.forecastCovers, d.staffHours, `£${d.revenue.toFixed(0)}`, `£${d.budget.toFixed(0)}`, `£${d.laborCost.toFixed(0)}`, `£${(d.budget - d.laborCost).toFixed(0)}`, d.weather ? `${d.weather.temp}°C` : 'N/A']);
    doc.autoTable({ startY: nextWeekStartY + 5, head: [['Date','Day','Covers','Hrs','Revenue','Budget','Labor','Variance','Weather']], body: nextWeekData, theme: 'grid', headStyles: { fillColor: [147,51,234], textColor: 255 }, styles: { fontSize: 8, cellPadding: 2 }, columnStyles: { 0: { cellWidth: 23 }, 1: { cellWidth: 18 }, 2: { cellWidth: 16, halign: 'right' }, 3: { cellWidth: 14, halign: 'right' }, 4: { cellWidth: 24, halign: 'right' }, 5: { cellWidth: 24, halign: 'right' }, 6: { cellWidth: 24, halign: 'right' }, 7: { cellWidth: 24, halign: 'right' }, 8: { cellWidth: 30 } } });

    const pageCount = doc.internal.getNumberOfPages();
    for (let i = 1; i <= pageCount; i++) {
      doc.setPage(i); doc.setFontSize(8); doc.setTextColor(156,163,175);
      doc.text(`Page ${i} of ${pageCount} | ${VENUES[venue].name} Forecast | Generated ${today}`, doc.internal.pageSize.width / 2, doc.internal.pageSize.height - 10, { align: 'center' });
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-purple-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-xl p-8 max-w-md w-full">
          ...
          {loginError && <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded text-red-700 text-sm">{loginError}</div>}
          <div className="flex justify-center">
            <GoogleLogin onSuccess={handleLoginSuccess} onError={handleLoginError} />
          </div>
          <p className="text-xs text-gray-500 text-center mt-6">Access restricted to @thefayreplay.co.uk</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                <button onClick={() => setActiveView('forecast')} className={`px-3 py-1 rounded-md text-sm mr-2 ${activeView === 'forecast' ? 'bg-indigo-100 text-indigo-700 font-bold' : 'text-gray-500 hover:bg-gray-100'}`}>Forecast</button>
                <button onClick={() => setActiveView('lookback')} className={`px-3 py-1 rounded-md text-sm mr-2 ${activeView === 'lookback' ? 'bg-indigo-100 text-indigo-700 font-bold' : 'text-gray-500 hover:bg-gray-100'}`}>Lookback (Actuals)</button>
                <button onClick={() => setActiveView('food-vendor')} className={`px-3 py-1 rounded-md text-sm ${activeView === 'food-vendor' ? 'bg-green-100 text-green-700 font-bold' : 'text-gray-500 hover:bg-gray-100'}`}>Food Vendor Report</button>
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleLogout} className="p-2 hover:bg-red-50 text-gray-400 hover:text-red-600 rounded-lg"><LogOut size={20} /></button>
          </div>
        </div>
      </header>
          ...
          <>
            <div className="bg-white p-4 rounded-xl border border-gray-200 mb-6 flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4 w-full md:w-auto">
                ...
                {rawData.length > 0 && (
                  <>
                    <button onClick={handleExport} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm border border-gray-200 bg-white hover:bg-gray-50 text-gray-700"><Download size={16} /> CSV</button>
                    <button onClick={handleExportPDF} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm border border-indigo-200 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-medium"><FileText size={16} /> PDF Report</button>
                  </>
                )}
              </div>
              {uploadStats && (
            ...
            <section className="mb-8">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                ...
                <SummaryCard title="Labor Variance" value={`£${(projection.summaries.thisWeek.budget - projection.summaries.thisWeek.laborCost).toFixed(0)}`} subtitle={projection.summaries.thisWeek.budget > projection.summaries.thisWeek.laborCost ? "Under Budget (Good)" : "Over Budget (Bad)"} status={projection.summaries.thisWeek.budget > projection.summaries.thisWeek.laborCost ? 'good' : 'bad'} />
              </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                ...
                <SummaryCard title="Labor Variance" value={`£${(projection.summaries.nextWeek.budget - projection.summaries.nextWeek.laborCost).toFixed(0)}`} subtitle={projection.summaries.nextWeek.budget > projection.summaries.nextWeek.laborCost ? "Under Budget (Good)" : "Over Budget (Bad)"} status={projection.summaries.nextWeek.budget > projection.summaries.nextWeek.laborCost ? 'good' : 'bad'} />
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
